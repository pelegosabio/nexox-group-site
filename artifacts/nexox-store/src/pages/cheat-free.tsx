...
export default function CheatFreePage() {
  ...
  useEffect(() => {
    fetch(`${BASE()}/api/downloads/free`)
      .then((r) => r.json())
      .then((d) => { if (d.downloadUrl) setDownloadUrl(d.downloadUrl); })
      .catch(() => {});
  }, []);
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        { sender: "admin", text: "Obrigado por enviar o print! Estamos verificando sua inscrição no canal. Aguarde um momento..." },
      ]);

      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          { sender: "admin", text: "Inscrição verificada com sucesso! Seu download foi liberado." },
        ]);
        setStep("download");
        setSending(false);
      }, 3000);
        ...
        <div className="w-full max-w-lg">
          {step === "instructions" && (
            <div className="rounded-[32px] border border-white/10 bg-white/5 backdrop-blur-2xl p-8">
              ...
              <div className="space-y-4 mb-8">
                {[
                  { n: "1", title: "Inscreva-se no canal", desc: "Acesse nosso canal no YouTube/Discord e se inscreva." },
                  { n: "2", title: "Tire um print da inscrição", desc: "Capture a tela mostrando que você está inscrito." },
                  { n: "3", title: "Envie o print no chat", desc: "Mande o print para verificação e libere o download." },
                ].map((s) => (
              ...
              <button onClick={() => setStep("chat")} className="w-full py-4 rounded-2xl bg-white text-black font-black hover:scale-[1.02] transition-all duration-300">
                Continuar
              </button>
            </div>
          )}
          ...
          {step === "chat" && (
            <div className="rounded-[32px] border border-white/10 bg-white/5 backdrop-blur-2xl overflow-hidden">
              ...
              <div className="p-6 space-y-4 min-h-[280px] max-h-[320px] overflow-y-auto">
                <div className="flex gap-3">
                  ...
                  <div className="bg-zinc-900 border border-white/10 rounded-2xl rounded-tl-none px-4 py-3 max-w-[80%]">
                    <p className="text-sm">Olá! Para liberar o CHEAT FREE, envie um print da sua inscrição no nosso canal.</p>
                  </div>
                </div>
                {messages.map((msg, idx) => (
                  <div key={idx} className={`flex gap-3 ${msg.sender === "user" ? "flex-row-reverse" : ""}`}>
                    ...
                    <div className={`rounded-2xl px-4 py-3 max-w-[80%] text-sm ${msg.sender === "user" ? "bg-white text-black rounded-tr-none" : "bg-zinc-900 border border-white/10 rounded-tl-none"}`}>
                      {msg.text && <p>{msg.text}</p>}
                      {msg.image && <img src={msg.image} alt="print" className="mt-2 rounded-xl max-w-full max-h-40 object-contain" />}
                    </div>
                  </div>
                ))}
                {sending && (
              ...
              <div className="px-6 py-4 border-t border-white/10 bg-black/20">
                ...
                <div className="flex gap-3">
                  <button onClick={() => fileInputRef.current?.click()} className="w-10 h-10 rounded-xl border border-white/10 bg-white/5 flex items-center justify-center hover:bg-white/10 transition shrink-0" title="Enviar imagem">
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                  </button>
                  <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
                  <input type="text" value={inputMessage} onChange={(e) => setInputMessage(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSendMessage()} placeholder="Digite ou cole um print (Ctrl+V)..." className="flex-1 px-4 py-2 rounded-xl bg-black/40 border border-white/10 text-white placeholder-zinc-600 focus:outline-none focus:border-white/30 transition text-sm" />
                  <button onClick={handleSendMessage} disabled={sending} className="px-4 py-2 rounded-xl bg-white text-black font-bold hover:scale-105 transition disabled:opacity-50 shrink-0 text-sm">
                    Enviar
                  </button>
                </div>
              </div>
          ...
          {step === "download" && (
            <div className="rounded-[32px] border border-white/10 bg-white/5 backdrop-blur-2xl p-8 text-center">
              ...
              <p className="text-zinc-400 mb-8 leading-relaxed">Sua inscrição foi verificada com sucesso. Clique abaixo para baixar o CHEAT FREE.</p>
              {downloadUrl ? (
                <a
                  href={downloadUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block w-full py-4 rounded-2xl bg-white text-black font-black hover:scale-[1.02] transition-all duration-300 mb-4 text-center"
                >
                  ⬇️ Baixar CHEAT FREE
                </a>
              ) : (
                <button onClick={() => alert("Link de download ainda não configurado. Entre em contato com o suporte.")} className="w-full py-4 rounded-2xl bg-white text-black font-black hover:scale-[1.02] transition-all duration-300 mb-4">
                  ⬇️ Baixar CHEAT FREE
                </button>
              )}
              <Link to="/" className="block text-zinc-400 hover:text-white transition text-sm">Voltar para a loja</Link>
            </div>
          )}
        </div>
      </div>
